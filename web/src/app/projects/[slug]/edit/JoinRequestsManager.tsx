...
export default function JoinRequestsManager({
  ...
  async function handleApprove(request: RequestWithVolunteer) {
    ...
    const { error: memberError } = await supabase
      ...
      .insert({ project_id: projectId, volunteer_id: request.volunteer_id, role_title: null, is_lead: false })
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {requests.map((r) => (
        <div key={r.id} className="flex items-start gap-3 border border-gray-200 rounded-lg p-4">
